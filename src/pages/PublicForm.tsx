...
import { formatPhone, formatCpf, formatCep, formatTituloEleitor } from '../utils/masks';
...
const RELACIONAMENTOS = ['Primeiro contato', 'Quero Acolhimento', 'Simpatizante', 'Apoiador', 'Liderança comunitária', 'Parceiro estratégico'];
...
export default function PublicForm() {
  ...
  const [cpfStatus, setCpfStatus] = useState<{ checking: boolean; exists: boolean; leaderName?: string }>({ checking: false, exists: false });
  ...
  useEffect(() => {
    ...
    if (digits.length !== 11) {
      setCpfStatus({ checking: false, exists: false });
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/public/leads/check-cpf/${digits}`);
        if (cancelled) return;
        const data = await res.json();
        if (!cancelled) setCpfStatus({ checking: false, exists: !!data.exists, leaderName: data.leaderName });
      } catch {
        if (!cancelled) setCpfStatus({ checking: false, exists: false });
      }
    }, 600);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!lgpdAgreed) {
      setError('Você deve concordar com o termo de consentimento LGPD para registrar o contato.');
      return;
    }
    if (cpfStatus.exists) {
      setError(`Este CPF já está cadastrado pela liderança ${cpfStatus.leaderName || 'desconhecida'}.`);
      return;
    }
    ...
    const finalContactOrigins = contactOrigins.includes('Outro') && customOrigin.trim()
      ? [...contactOrigins.filter(s => s !== 'Outro'), customOrigin.trim()]
      : contactOrigins;

    const finalSegments = segments.includes('Outro') && customSegment.trim()
      ? [...segments.filter(s => s !== 'Outro'), customSegment.trim()]
      : segments;

    const finalInfluencePotentials = influencePotentials.includes('Outro') && customInfluence.trim()
      ? [...influencePotentials.filter(s => s !== 'Outro'), customInfluence.trim()]
      : influencePotentials;

    const finalNextActions = nextActions.includes('Outro') && customAction.trim()
      ? [...nextActions.filter(s => s !== 'Outro'), customAction.trim()]
      : nextActions;
      ...
      const res = await fetch('/api/public/leads', {
        ...
        headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...
            city,
            contactOrigins: finalContactOrigins,
            segments: finalSegments,
            relationshipLevels,
            influencePotentials: finalInfluencePotentials,
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-10">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input required type="tel" value={formatPhone(phone)} onChange={(e) => setPhone(e.target.value.replace(/\D/g, '').slice(0, 11))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="(00) 00000-0000" />
              </div>
              <div>
              ...
              <div>
                ...
                <input required type="text" value={formatCpf(cpf)} onChange={(e) => setCpf(e.target.value.replace(/\D/g, '').slice(0, 11))} className={`w-full px-4 py-3 bg-slate-50 border ${cpfStatus.exists ? 'border-red-300' : 'border-slate-200'} rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none`} placeholder="000.000.000-00" />
                {cpfStatus.checking && <p className="text-xs text-indigo-600 mt-1">Verificando CPF...</p>}
                {cpfStatus.exists && (
                  <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                    <AlertCircle className="w-3 h-3 shrink-0" /> Este CPF já está cadastrado pela liderança <strong>{cpfStatus.leaderName || 'desconhecida'}</strong>.
                  </p>
                )}
              </div>
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-1">
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">
                    Título de Eleitor – Nº de Inscrição
                  </label>
                  <input type="text" value={formatTituloEleitor(voterTitle)} onChange={(e) => setVoterTitle(e.target.value.replace(/\D/g, '').slice(0, 12))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="XXXX.XXXX.XXXX" />
                </div>
                <div className="grid grid-cols-2 gap-4 md:contents">
                  <div>
                    ...
                    <input type="text" value={voterZone} onChange={(e) => setVoterZone(e.target.value.replace(/\D/g, '').slice(0, 3))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="001" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">
                      Seção
                    </label>
                    <input type="text" value={voterSection} onChange={(e) => setVoterSection(e.target.value.replace(/\D/g, '').slice(0, 4))} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none" placeholder="0001" />
                  </div>
                </div>
              </div>
              <div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  value={administrativeRegions[0] || ''}
                  onChange={(e) => setAdministrativeRegions(e.target.value ? [e.target.value] : [])}
          ...
          <section>
            ...
            {contactOrigins.includes('Outro') && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Como esse contato foi realizado? <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={customOrigin}
                  onChange={(e) => setCustomOrigin(e.target.value)}
                  placeholder="Digite a origem do contato"
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none"
                />
              </div>
            )}
          ...
          <section>
            ...
            {segments.includes('Outro') && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Qual segmento? <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={customSegment}
          ...
          <section>
            ...
            {influencePotentials.includes('Outro') && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Qual o potencial de influência? <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={customInfluence}
                  onChange={(e) => setCustomInfluence(e.target.value)}
                  placeholder="Digite o potencial de influência"
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600 outline-none"
                />
              </div>
            )}
          ...
          <section>
            ...
            {nextActions.includes('Outro') && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Qual o próximo passo? <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={customAction}
          ...
          <section>
            <h2 className="text-xl font-semibold text-slate-800 mb-4 pb-2 border-b border-slate-200 flex items-center gap-2">
              <FileText className="h-5 w-5 text-indigo-600" /> BLOCO 9 – CONSENTIMENTO LGPD
            </h2>
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
              <p className="text-sm text-slate-700 mb-4 leading-relaxed">
                Autorizo a Equipe MB a registrar e tratar os dados pessoais informados (nome, CPF, telefone, e-mail e endereço) exclusivamente para fins de cadastro, comunicação e mobilização da campanha, nos termos da Lei Geral de Proteção de Dados (Lei nº 13.709/2018). Estou ciente de que posso solicitar, a qualquer momento, a correção ou a exclusão dos meus dados.
              </p>
